...
const A = 'squads/carrossel-vitaegold/output/_build/produto-colagenoc2/assets';
const OUT = 'squads/carrossel-vitaegold/output/produto-colagenoc2/images';
mkdirSync(OUT, { recursive: true });
const b64 = (p, m) => `data:${m};base64,` + readFileSync(`${A}/${p}`).toString('base64');
const LOGO = b64('logo-nova.png', 'image/png');
const PROD = b64('produto.png', 'image/png');
...
h1.brand{font-size:72px;margin-top:14px}
...
.quote{font-family:'Fraunces';font-weight:500;font-style:italic;font-size:46px;line-height:1.2;color:${INK};margin-top:18px}
...
function heroSlide(n){return `<div class="stage">
  <div class="prodband"><div class="prodcard"><img src="${PROD}"></div></div>
  <div class="chip"><img src="${LOGO}"></div>
  <div class="sheet h650"><div class="row"><div class="rule"></div>${dots(n)}</div>
  <div class="tag">Mobilidade & Conforto</div>
  <h1 class="brand">Colágeno C2</h1>
  <div class="sub">Colágeno tipo II não desnaturado para as articulações</div>
  <div class="foot"><span class="num">0${n} / 0${TOTAL}</span><span class="sw">arraste ${swipe()}</span></div></div>
</div>`;}

function photoSlide(n,s){return `<div class="stage">
  <div class="photoband" style="background-image:url('${b64(s.img,'image/jpeg')}');background-position:${s.pos}"></div>
...
function quoteSlide(n,s){return `<div class="stage">
  <div class="prodband"><div class="prodcard"><img src="${PROD}"></div></div>
  <div class="chip"><img src="${LOGO}"></div>
  <div class="sheet h650"><div class="row"><div class="rule"></div>${dots(n)}</div>
  <div class="pill">Quem usa</div>
  <p class="quote">${nl(s.quote)}</p>
  <div class="qfrom">${esc(s.from)}</div>
  <div class="foot"><span class="num">0${n} / 0${TOTAL}</span><span class="sw">arraste ${swipe()}</span></div></div>
</div>`;}

function ctaSlide(n,s){return `<div class="stage">
  <div class="chip"><img src="${LOGO}"></div>
  <div class="sheet h760" style="height:1350px;border-radius:0;justify-content:flex-start;padding-top:230px">
  <div class="row"><div class="rule"></div>${dots(n)}</div>
  <div class="tag">Comece hoje</div>
  <h1>${nl(s.headline)}</h1><p class="body">${nl(s.body)}</p>
  <div class="btn">${wa()} Pedir no WhatsApp</div>
  <div class="phone">ou ligue 0800 888 0032</div>
  <div class="aviso">${esc(s.aviso)}</div></div>
</div>`;}

const slides = [
  { type:'hero' },
  { type:'photo', img:'life-horta.jpg', pos:'center 35%', pill:'Rotina ativa',
    headline:'Feito pra quem não quer parar.',
    body:'O Colágeno C2 tem colágeno tipo II não desnaturado em cápsulas, pra complementar a alimentação de quem cuida da mobilidade no dia a dia.' },
  { type:'photo', img:'life-caminhada.jpg', pos:'center 35%', pill:'Função articular',
    headline:'Auxilia na manutenção da função articular.',
    body:'O colágeno tipo II não desnaturado é o foco da fórmula. Direto ao ponto, sem encher de aditivos.' },
  { type:'quote', quote:'"Tomo todo dia antes do almoço. Não tem gosto, passa fácil."', from:'Relato de quem já incluiu o Colágeno C2 na rotina' },
  { type:'cta',
    headline:'Cuide das suas articulações\ntodo dia.',
    body:'Fale com a gente no WhatsApp e comece sua rotina com o Colágeno C2, com condições especiais em kits.',
    aviso:'Suplemento alimentar. Este produto não é um medicamento e não deve ser utilizado para tratamento, cura ou prevenção de doenças. Não substitui uma alimentação equilibrada e variada e hábitos de vida saudáveis. O colágeno tipo II não desnaturado auxilia na manutenção da função articular.' },
];
...
<style>${CSS}</style></head><body>${s}</body></html>`;
...
for (const s of slides) {
  ...
  writeFileSync(`${A}/_s${n}.html`, html(inner));
...
await browser.close(); console.log('DONE produto');
